...
// use libfabric::mr::{MemoryRegionDesc, MemoryRegionKey, MappedMemoryRegionKey, MemoryRegion};
// use libfabric::info::InfoEntry;

// use super::transport::reg_mr;
// extern crate memmap;
...
//     pub(crate) fn new(mem: memmap::MmapMut, mr: MemoryRegion, desc: MemoryRegionDesc, key: MemoryRegionKey) -> Self {
...
//     pub(crate) fn get_remote_key(&self, remote_id: usize) -> &Rc<MappedMemoryRegionKey> {
...
//     pub(crate) fn alloc<I: Caps>(&mut self, info: &InfoEntry<I>, domain: &Domain, ep: &Endpoint<I>, size: usize) ->  Rc<MappedMemoryRegion> {
...
//         let (mr, mr_desc, key) = reg_mr(info, domain, ep, &mut mem, self.mr_next_key).unwrap();
