...
use libfabric::{av::AddressVector, cntr::Counter, cq::CompletionQueue, domain::Domain, enums::MrMode, ep::{EndpointAttr, Endpoint}, eq::{EventQueue, EventQueueAttr}, error::Error, fabric::Fabric, mr::MemoryRegionDesc, Address, CollectiveAttr, Info, InfoEntry, RmaIoVec, FID};
...
pub struct Rofi {                       // Note that the order in which libfabric structs are defined matters 
    ...
    mr_tab: Vec<std::rc::Rc<mr::MappedMemoryRegion>>,
    ep: Endpoint,
    eq: EventQueue,
    tx: XxData,
    rx: XxData,
...
impl Rofi {
    ...
    pub(crate) fn init(pmi: Box<dyn crate::pmi::PmiTrait>) -> Result<Rofi, libfabric::error::Error> {
        ...
        let mut addresses: Vec<Address> =  vec![u64::MAX; pmi.get_size() as usize];
        let mut addr  = vec![0_u8; 16];

        let len = ep.getname(&mut addr)?;

        pmi.put("epname", &addr[0..len]);
        pmi.exchange();
        let mut all_addresses: Vec<u8> = vec![0 ; len * pmi.get_size() as usize ];
        for i in 0..pmi.get_size() as usize {
            let res = pmi.get(i, "epname", len);
        ...
        let mut rofi = Rofi {
            world: CommWorld{ nnodes: pmi.get_size() as usize, my_id: pmi.get_rank() as usize, addresses},
            pmi,
            all_info,
            info,
            fabric,
            ...
            mr_next_key: 0,
            mr_tab: Vec::new(),
            ctx_bank: ContextBank::new(),
            barrier_mr_index : 0, 
            barrier_id : 0,
        };

        let barrier_size = rofi.world.nnodes * std::mem::size_of::<usize>();
        let _barrier_mr = rofi.alloc(barrier_size);
    ...
    pub fn alloc(&mut self, size: usize) ->  std::rc::Rc<mr::MappedMemoryRegion> {
        ...
        let (mr, mr_desc, key) = transport::reg_mr(&self.info, &self.domain, &self.ep, &mut mem, self.mr_next_key).unwrap();

        self.mr_next_key += 1;

        debug_println!("P[{}] Allocated mapped memory with start: {} and end: {}", self.world.my_id, mem.as_ptr() as usize, mem.last().unwrap() as *const u8 as usize);
        let remote_iovs = self.exchange_keys(mem.as_ptr() as u64, key);
        
        debug_println!("P[{}] Global Mapped memories found:", self.world.my_id);
        for (i, iov) in remote_iovs.iter().enumerate() {
            debug_println!("\tP[{}] {}", i, iov.get_address());
        }

        let res = std::rc::Rc::new(mr::MappedMemoryRegion::new(mem, mr, mr_desc, key, remote_iovs));
        self.mr_tab.push(res.clone());
        
        res.clone()
    }

    pub fn sub_alloc(&mut self, size: usize, pes: &[usize]) ->  std::rc::Rc<mr::MappedMemoryRegion> {
        ...
        let remote_iovs = self.sub_exchange_mr_info(mem.as_ptr() as u64, key, pes);

        let res = std::rc::Rc::new(mr::MappedMemoryRegion::new_sub(mem, mr, mr_desc, key, remote_iovs, pes));
        self.mr_tab.push(res.clone());
    ...
    unsafe fn put_(&mut self, mut dst: usize, src: &[u8], id: usize, block: bool) -> Result<(), std::io::Error> {
        ...
        if std::mem::size_of_val(src) < self.info.get_tx_attr().get_inject_size() {
            ...
            let ctx_id = self.ctx_bank.create();
            let mut curr_idx = 0;
            
            while curr_idx < src.len() {
                let msg_len = std::cmp::min(src.len() - curr_idx, self.info.get_ep_attr().get_max_msg_size()); 

                self.post_rma(&RmaOp::RmaWrite, &rma_iov, &src[curr_idx..curr_idx+msg_len], &mut mem.get_mr_desc(), id, ctx_id);
                dst += msg_len;
                curr_idx += msg_len;
    ...
    unsafe fn get_(&mut self, mut src: usize, dst: &mut[u8], id: usize, block: bool) -> Result<(), std::io::Error> {
        ...
        while curr_idx < dst.len() {
            ...
            self.post_rma_mut(&RmaOp::RmaRead, &rma_iov, &mut dst[curr_idx..curr_idx+msg_len], &mut mem.get_mr_desc(), id, ctx_id);
            src += msg_len;
            curr_idx += msg_len;
    ...
    fn wait_get_all(&mut self) -> Result<(), Error> {
        ...
        transport::wait_on_cntr(&mut self.rx.cq_seq, &mut self.rx.cntr)
    }

    fn wait_put_all(&mut self) -> Result<(), Error> {

        transport::wait_on_cntr(&mut self.tx.cq_seq, &mut self.tx.cntr)
    }

    fn mr_get(&self, addr: usize) -> Option<std::rc::Rc<mr::MappedMemoryRegion>>{
        
        self.mr_tab.iter().find(|x| x.contains(addr)).cloned()
    }

    #[allow(dead_code)]
    fn mr_get_from_remote(&self, addr: usize, remote_id: usize) -> Option<std::rc::Rc<mr::MappedMemoryRegion>> {

        self.mr_tab.iter().find(|x| x.remote_contains(remote_id, addr)).cloned()
    }

    #[allow(dead_code)]
    fn mr_rm(&mut self, addr: usize) {
        self.mr_tab.remove(self.mr_tab.iter().position(|x| x.get_start() == addr).expect("Address to remove not found"));
    }
    ...
    fn exchange_keys(&mut self, addr: libfabric::Address, key: u64) -> Vec<RmaIoVec> {
        ...
        let raw_iovs = self.pmi.exchange_data("ep_addr", raw_iov);
    ...
    unsafe fn  post_rma_mut(&mut self, rma_op: &RmaOp, remote: &libfabric::RmaIoVec, buf: &mut [u8], mr_desc: &mut MemoryRegionDesc, id: usize, ctx_id: usize) {
        let remote_address = self.world.addresses[id];
        let ctx = self.ctx_bank.get(ctx_id).unwrap();
    ...
    unsafe fn  post_rma(&mut self, rma_op: &RmaOp, remote: &libfabric::RmaIoVec, buf: &[u8], mr_desc: &mut MemoryRegionDesc,  id: usize, ctx_id: usize) {
        let remote_address = self.world.addresses[id];
        let ctx = self.ctx_bank.get(ctx_id).unwrap();
    ...
    pub fn flush(&mut self) {
        transport::progress(&mut self.tx.cq, self.tx.cq_seq, &mut self.tx.cq_cntr);
        transport::progress(&mut self.rx.cq, self.rx.cq_seq, &mut self.rx.cq_cntr);
    }
    ...
    pub fn barrier(&mut self) {
        ...
        for round in 0..num_rounds as usize {
            for i in 1..=n {
                let send_pe = euclid_rem(self.world.my_id  as i64 + i  as i64 * (n as i64 + 1 ).pow(round as u32) as i64, self.world.nnodes as i64 );
                
                let dst = self.mr_tab[self.barrier_mr_index].get_mem().borrow().as_ptr()  as usize + 8 * self.world.my_id;
            ...
            for i in 1..=n {
                let recv_pe = euclid_rem(self.world.my_id as i64 - i as i64 * (n  as i64 + 1).pow(round as u32) as i64 , self.world.nnodes as i64);
                let barrier_vec = unsafe {std::slice::from_raw_parts(self.mr_tab[self.barrier_mr_index].get_mem().borrow().as_ptr() as *const usize,  self.world.nnodes) };
    ...
    pub fn sub_barrier(&mut self, pes: &[usize]) {
        ...
        for round in 0..num_rounds as usize {
            for i in 1..=n {
                let send_pe = euclid_rem(self.world.my_id as i64 + i as i64 * (n as i64 + 1).pow(round as u32) as i64, num_pes as i64 );
            ...
            for i in 1..=n {
                let recv_pe = euclid_rem(self.world.my_id as i64 - i as i64 * (n as i64 + 1).pow(round as u32) as i64, num_pes as i64 );
                let recv_pe = pes[recv_pe as usize];
                let barrier_vec = unsafe {std::slice::from_raw_parts(self.mr_tab[self.barrier_mr_index].get_mem().borrow().as_ptr() as *const usize,  8 * self.world.nnodes) };
    ...
    fn sub_exchange_mr_info(&mut self, addr: u64, key: u64, pes: &[usize]) -> Vec<RmaIoVec> {
        ...
        for i in 1..pes.len() {
            av_set.insert(self.world.addresses[pes[i]]).unwrap();
        }
